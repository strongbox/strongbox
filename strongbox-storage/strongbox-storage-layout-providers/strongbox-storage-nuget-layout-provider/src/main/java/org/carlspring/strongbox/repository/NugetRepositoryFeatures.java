        ...
        implements RepositoryFeatures
{
            ...
            throws IOException
    {
        ...
        try
        {
            ...
            logger.debug("Downloaded remote feed for [{}], size [{}].",
                remoteRepository.getUrl(), Optional.of(packageFeed).map(f -> f.getEntries().size()).orElse(0));
    ...
    public class RepositorySearchEventListener
    {
        ...
        public void handle(RemoteRepositorySearchEvent event) throws IOException
        {
            ...
            logger.debug("Remote repository [{}] cached package count is [{}]",
            ...
            try
            {
                ...
                Long remotePackageCount = Long.valueOf(queryParams(service.path("Search()/$count"),
                                                                   nugetSearchRequest, new Paginator()).request()
                                                                                                       .buildGet()
                                                                                                       .invoke(String.class));
                logger.debug("Remote repository [{}] remote package count is [{}]",
                ...
                logger.debug("Downloaded remote feed for [{}], size [{}].",
                    remoteRepository.getUrl(), Optional.of(feed).map(f -> f.getEntries().size()).orElse(0));
